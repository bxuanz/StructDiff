...
def main(args):
    ...
    model_id_or_path = "/data01/zhaobingxuan/training_free/RichControl/models--stabilityai--stable-diffusion-xl-base-1.0/snapshots/462165984030d82259a11f4367a4eed129e94a7b"
    # refiner_id_or_path = "/data01/zhaobingxuan/training_free/RichControl/models--stabilityai--stable-diffusion-xl-refiner-1.0/snapshots/5d4cfe854c9a9a87939ff3653551c2b3c99a4356"
    ...
    scheduler = RichControlDDIMScheduler.from_pretrained(model_id_or_path, subfolder="scheduler")
    ...
    if model_config.get('modify_prompt', False):
        print("Loading local LLaVA model for Appearance-Rich Prompting...")
        try:
            ...
            bnb_config = BitsAndBytesConfig(
                ...
                bnb_4bit_compute_dtype=torch_dtype # This should be torch.float16
            )
            ...
            llava_processor = AutoProcessor.from_pretrained(llava_model_id, use_fast=True)
            print("LLaVA model loaded successfully.")

        except ImportError:
            raise ModuleNotFoundError("`transformers` and `bitsandbytes` must be installed to use the local LLaVA model. Please run: pip install transformers bitsandbytes")
        except Exception as e:
            print(f"Could not load LLaVA model, ARP will be skipped. Error: {e}")
            model_config['modify_prompt'] = False # Disable if loading fails
    ...
    # model_config is already loaded
    ...
    if model_config.get('modify_prompt', False):
        appearance_prompt = compose_prompt_llava_rs(
            model=llava_model,
        ...
        # Print the enriched prompt to the terminal
        print("\n" + "="*50)
        print("✨ Enriched Prompt (ARP):")
        print(f"   {appearance_prompt}")
        print("="*50 + "\n")
    # <<< MODIFICATION END >>>
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_config", "-rc", type=str, default="./configs/model_config_llava.yaml")
    parser.add_argument("--output_folder", "-o", type=str, default="./results/00060/llava/canny")
